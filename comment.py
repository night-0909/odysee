...
# Note : autoindent package is https://github.com/ANoneTypeOn/autoindent/blob/master/autoindent.py // with small change
...
class Program():
    ...
    def writeComments(self, comments, indent=0):
        for num, comment in enumerate(comments, start=1):
            ...
            release_time = int(comment.get("timestamp"))
            publish_date_comment = datetime.fromtimestamp(release_time, self.tzinfo).strftime(self.dateFormats['dateString'])
            signing_ts = int(comment.get("signing_ts"))
            if signing_ts > comment.get("timestamp"):
                signing_ts_comment =  datetime.fromtimestamp(signing_ts, self.tzinfo).strftime(self.dateFormats['dateString'])   
                publish_date_comment += " (last edited on : " + signing_ts_comment + ")"

            line = publish_date_comment + " " + ch_name + " " + "(" + ch_id + ") : " + comm
            indent_line = Indent()
