    ...
    def main(self):
        ...
        while hasMorePages is True:
            ...
            for item in items:               
                ...
                if "release_time" in item.get('value'):
                    dateContent = item.get('value').get('release_time')
                else:
                    dateContent = item.get("timestamp")

                dateContent_text = datetime.fromtimestamp(int(dateContent), self.tzinfo).strftime(self.dateFormats['dateString'])

                claim_type = item.get('value_type')
                reposted_claim = item.get('reposted_claim')

                value = None
                claim_id_additionnalreq = None
                if claim_type == 'repost':
                    # reposted_claim can be missing here
                    if reposted_claim is not None:
                ...
                if value is not None:
                    ...
                    # For video and audio, we get duration of file
                    if stream_type in ('video', 'audio'):
                        duration = value.get(stream_type).get('duration')
                ...
                commentCount = None
                threads = []
                if auth_token is not None and claim_id_additionnalreq is not None:
                    # View count
                ...
                # Comment count
                if claim_id_additionnalreq is not None:
                ...
                print("Date : " + dateContent_text)
                self.writeresult("Date : " + dateContent_text)

                if self.getThumbnail is True and value is not None:
                    # Thumbnail can be empty (playlist)
                    if value.get('thumbnail') is not None:
                        ...
                        try:
                            ...
                            if response.status_code == 200:
                                ...
                                filethumbnail = claim_id + "_thumbnail_" + datetime.fromtimestamp(datetime.now().timestamp(), self.tzinfo).strftime(self.dateFormats['dateFileString'])
                                ...
                                if fileExtension is not None:
                                    ...
                                    print(f"[×] claim_id={claim_id_additionnalreq} filethumbnail={filethumbnail} Couldn't guess file extension mimetype={fileMimetype}, we keep file without extension")
                                    self.writelog(f"[×] claim_id={claim_id_additionnalreq} filethumbnail={filethumbnail} Couldn't guess file extension mimetype={fileMimetype}, we keep file without extension")
                            else:
                ...
                if claim_type == 'repost' and reposted_claim is not None:                   
                    self.writeresult("\nOriginal content :\n")
                    ...
                    if "release_time" in reposted_claim.get('value'):
                        dateContent = reposted_claim.get('value').get('release_time')
                    else:
                        dateContent = reposted_claim.get("timestamp")
                    
                    self.writeresult("Date original content : " + datetime.fromtimestamp(int(dateContent), self.tzinfo).strftime(self.dateFormats['dateString']))
                    self.writeresult("\n")

                    if reposted_claim.get('signing_channel').get('canonical_url') is not None:
                        canonical_url = reposted_claim.get('signing_channel').get('canonical_url').replace('lbry://@', 'https://odysee.com/@').replace('#', ':')
                        channel_id = reposted_claim.get('signing_channel').get('claim_id')
                    else:
                        canonical_url = "Anonymous"
                        channel_id = reposted_claim.get('signing_channel').get('channel_id')
                    
                    self.writeresult("Author : " + canonical_url + " (" + channel_id + ")")
                    self.writeresult("\n")
